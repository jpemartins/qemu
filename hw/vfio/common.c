...
static int vfio_get_max_iova(hwaddr *max_iova)
{
    VFIOAddressSpace *space;
    MemoryRegion *mr;
    int ret = -EINVAL;
    ...
    QLIST_FOREACH(space, &vfio_address_spaces, list) {
        ...
        mr = space->as->root;
        ret = 0;
        *max_iova = mr->addr + memory_region_size(mr) - 1;
    }
...
static void vfio_dirty_tracking_update(hwaddr iova, hwaddr end, VFIODirtyRanges *range)
{
    hwaddr *min, *max;
                                        ...
                                        MemoryRegionSection *section)
{
    ...
    if (vfio_devices_all_device_dirty_tracking(container)) {
        ...
        ret = vfio_devices_query_dirty_bitmap(container, &gdn.vbmap, start, size);
        if (ret) {
